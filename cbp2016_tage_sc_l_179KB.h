...
//global branch GEHL
#define LOGGNB 11       // 1 1K + 2 * 512-entry tables
#define GNB 3
int Gm[GNB] = { 40, 24, 10 };
...
#define PNB 3
#define LOGPNB 10        // 1 1K + 2 * 512-entry tables
int Pm[PNB] = { 25, 16, 9 };
...
#define NBANKLOW 15     // number of banks in the shared bank-interleaved for the low history lengths
#define NBANKHIGH 30        // number of banks in the shared bank-interleaved for the  history lengths
...
int predictorsize ()
{
    ...
    fprintf (stdout, " (TOTAL %d bits %d Kbits) ", STORAGESIZE,
            STORAGESIZE / 1024);
#endif
    fprintf (stdout, " (TOTAL %d bits %d KB) ", STORAGESIZE,
            STORAGESIZE / (8 * 1024));
